...
app.get("/get_todos", async(req,res)=>{
    try {
        const get_todos = await pool.query("SELECT * FROM todo");//no returning cause Select already returns
...
try {
    ...
    const get_todos = await pool.query("SELECT * FROM todo WHERE todo_id = $1",[id]);
...
app.delete('/remove_todo/:id', async(req,res)=>{
    try {
        const {id} = req.params;
        const remove_todo = await pool.query("DELETE FROM todo WHERE todo_id = $1",[id]);
